...
// ── Timezone helpers ──────────────────────────────────────────────
const formatInTZ = (dateStr, tz, opts) => {
    const d = new Date(dateStr);
    if (isNaN(d)) return '—';
    return new Intl.DateTimeFormat('en-GB', { timeZone: tz, ...opts }).format(d);
};

const toUKTime = (dateStr) => {
    const d = new Date(dateStr);
    if (isNaN(d)) return '—';
    const time = formatInTZ(dateStr, 'Europe/London', { hour: '2-digit', minute: '2-digit', hour12: false });
    const day  = formatInTZ(dateStr, 'Europe/London', { day: '2-digit', month: 'short' });
...
const toUSCentral = (dateStr) => {
    const d = new Date(dateStr);
    if (isNaN(d)) return '—';
    const time = formatInTZ(dateStr, 'America/Chicago', { hour: '2-digit', minute: '2-digit', hour12: true });
    const day  = formatInTZ(dateStr, 'America/Chicago', { day: '2-digit', month: 'short' });
...
// ── Key helper: YYYY-MM-DD string from a Date in local time ───────
const toDateKey = (d) => {
...
// ── Get upcoming occurrences of an event ─────────────────────────
const DAY_MAP = {
...
const getUpcomingDates = (ev, weeksAhead = 14) => {
    ...
    if (freq === 'Daily') {
        // Generate every day from today for 30 days
        const d = new Date(todayStart);
        const limit = new Date(todayStart);
        limit.setDate(limit.getDate() + 30);
        while (d <= limit) {
            const dt = new Date(d);
            dt.setHours(baseHour, baseMin, 0, 0);
    ...
    // Monthly / One-time / other — just show base date if upcoming
    if (base >= todayStart) results.push(base);
    return results;
};

// ── Build calendar map: dateKey → list of events ─────────────────
const buildCalendarMap = (events) => {
    const map = {};
    events.forEach(ev => {
        const dates = getUpcomingDates(ev, 16);
        dates.forEach(d => {
...
const eventOccursOnDate = (ev, targetDate) => {
    ...
    return dates.some(d => {
        const dd = new Date(d);
        dd.setHours(0, 0, 0, 0);
        return toDateKey(dd) === targetKey;
    });
};

// ── Build display datetime for a specific date ────────────────────
const buildDisplayDateTime = (ev, targetDate) => {
    const base = new Date(ev.event_datetime);
        ...
        <div className="satsang-calendar">
            ...
            <div className="cal-grid">
                ...
                {Array.from({ length: daysInMonth }).map((_, i) => {
                    ...
                    return (
                        ...
                        >
                            {/* Today gets a distinct filled circle */}
            ...
            <div className="cal-legend">
                <span className="legend-item">
                    ...
                    <span>= events on that day (click to view)</span>
                </span>
...
const SatsangPage = () => {
    ...
    const filteredEvents = events.filter(ev => {
        // Date filter — use the fixed eventOccursOnDate
    ...
    // Get the display datetime for each event (next occurrence or selected date)
    const getDisplayDateStr = (ev) => {
        if (selectedDate) return buildDisplayDateTime(ev, selectedDate);
                ...
                <div className="ashram-247-banner">
                    <div className="ashram-247-left">
                        ...
                        <div>
                            ...
                            <p className="ashram-247-note">
                                Immerse in the eternal stream of Bhagawan's Divine Name — flowing ceaselessly, day and night, for seekers across the world.
                            </p>
                        </div>
                    </div>
                ...
                <section className="satsang-section" ref={tableRef}>
                    <div className="satsang-table-header">
                        <h2 className="satsang-section-title">
                            {selectedLabel
                                ? <>🗓 Events on {selectedLabel} <span className="event-count-badge">{filteredEvents.length}</span></>
                                : <>🌏 All Events <span className="event-count-badge">{filteredEvents.length}</span></>
                            }
                        </h2>
                        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                            {selectedDate && (
                                <button className="filter-clear-btn" onClick={clearFilters}>✕ Show All</button>
                            )}
                            ...
                            <table className="satsang-table">
                                ...
                                <tbody>
                                    ...
                                    {filteredEvents.length === 0 ? (
                                        ...
                                        const displayStr = getDisplayDateStr(ev);
                                        const ist        = formatIST(displayStr);
                                        ...
                                        return (
                                            <tr key={ev.id}>
                                                ...
                                                <td className="td-tz">
                                                    <span className="tz-line">{toUKTime(displayStr)}</span>
                                                </td>
                                                <td className="td-tz">
                                                    <span className="tz-line">{toUSCentral(displayStr)}</span>
                                                </td>
                                                <td>
